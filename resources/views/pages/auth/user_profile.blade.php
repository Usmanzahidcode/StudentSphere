                ...
                <ul class="list-group">
                    @foreach ($user->projects as $project)
                        <li class="list-group-item">
                            ...
                            <span class="badge bg-secondary ms-2">{{ ucfirst(str_replace('_', ' ', $project->status->value)) }}</span>
                ...
                <ul class="list-group">
                    @foreach ($user->participatedProjects as $project)
                        <li class="list-group-item">
                            ...
                            <span class="badge bg-success ms-2">{{ ucfirst(str_replace('_', ' ', $project->status->value)) }}</span>
                            <small class="text-muted d-block">Joined On: {{ now()->format('d M Y') }}</small>
                ...
                <ul class="list-group">
                    @foreach ($user->opportunities as $opportunity)
                        <li class="list-group-item">
                            ...
                            <span class="badge bg-warning ms-2">{{ ucfirst(str_replace('_', ' ', $opportunity->status->value)) }}</span>
